...
# --- IMPORTAÇÕES DE SERVIÇOS REMOVIDAS DO TOPO ---
# (Isso corrige o RuntimeError durante a coleta)

#pytestmark = pytest.mark.django_db # <-- ESTA É A CORREÇÃO FALTANTE
...
def test_get_professor_project_counts_success(mocker):
    ...
    """
    from src.core.services import get_professor_project_counts # <-- IMPORT AQUI
    
    # 1. ARRANGE
    fake_professor = mocker.MagicMock(id_professor=3937, nome="Prof. Teste")
    
    # --- CAMINHO DO PATCH CORRIGIDO ---
    mocker.patch(
        'src.core.repositories.get_professor_by_id', # <-- MUDANÇA AQUI
        return_value=fake_professor
    )
    mocker.patch(
        'src.core.repositories.get_active_orientador_count', # <-- MUDANÇA AQUI
        return_value=5
    )
    mocker.patch(
        'src.core.repositories.get_active_assessor_count', # <-- MUDANÇA AQUI
        return_value=2
    )
...
def test_get_professor_project_counts_not_found(mocker):
    ...
    with pytest.raises(ObjectDoesNotExist) as exc_info:
        get_professor_project_counts(professor_id=999)
...
def test_create_project_with_associations_existing_users(mocker):
    ...
    """
    from src.core.services import create_project_with_associations # <-- IMPORT AQUI
    ...
    # --- CAMINHOS DO PATCH CORRIGIDOS ---
    mock_get_prof = mocker.patch('src.core.repositories.get_professor_by_id', return_value=fake_professor) # <-- MUDANÇA AQUI
    mock_get_aluno = mocker.patch('src.core.repositories.get_aluno_by_id', return_value=fake_aluno) # <-- MUDANÇA AQUI
    mock_create_proj = mocker.patch('src.core.repositories.create_project', return_value=fake_project) # <-- MUDANÇA AQUI
    mock_create_orient = mocker.patch('src.core.repositories.create_orientador_assoc') # <-- MUDANÇA AQUI
    mock_create_alunoproj = mocker.patch('src.core.repositories.create_alunoproj_assoc') # <-- MUDANÇA AQUI
    ...
    # A função create_project espera os dados filtrados (Correção 1)
    mock_create_proj.assert_called_once_with({'tema': 'Novo Projeto de Teste', 'tipo': 1, 'resumo': '...', 'duracao': 12})
...
def test_create_project_with_new_orientador(mocker):
    ...
    # --- CAMINHOS DO PATCH CORRIGIDOS ---
    mock_create_prof = mocker.patch('src.core.repositories.create_professor', return_value=fake_new_professor) # <-- MUDANÇA AQUI
    mock_get_aluno = mocker.patch('src.core.repositories.get_aluno_by_id', return_value=fake_aluno) # <-- MUDANÇA AQUI
    mock_create_proj = mocker.patch('src.core.repositories.create_project', return_value=fake_project) # <-- MUDANÇA AQUI
    mock_create_orient = mocker.patch('src.core.repositories.create_orientador_assoc') # <-- MUDANÇA AQUI
    mock_create_alunoproj = mocker.patch('src.core.repositories.create_alunoproj_assoc') # <-- MUDANÇA AQUI
...
def test_create_project_professor_not_found(mocker):
    ...
    # 1. ARRANGE
    fake_data = {'id_professor': 999, 'id_aluno': 202} 
    ...
    mocker.patch(
        'src.core.repositories.get_aluno_by_id', # <-- MUDANÇA AQUI
...
def test_create_project_aluno_not_found(mocker):
    ...
    # 1. ARRANGE
    fake_data = {'id_professor': 101, 'id_aluno': 999} 
...
def test_associate_assessor_success(mocker):
    ...
    """
    from src.core.services import associate_assessor_to_project # <-- IMPORT AQUI
    ...
    # --- CAMINHOS DO PATCH CORRIGIDOS ---
    mocker.patch('src.core.repositories.get_professor_by_id', return_value=fake_professor) # <-- MUDANÇA AQUI
    mocker.patch('src.core.repositories.get_project_by_id', return_value=fake_project) # <-- MUDANÇA AQUI
    mocker.patch('src.core.repositories.check_if_orientador_is_assessor', return_value=False) # <-- MUDANÇA AQUI
    mock_create_assoc = mocker.patch('src.core.repositories.create_assessor_assoc') # <-- MUDANÇA AQUI
...
def test_associate_assessor_fails_if_is_orientador(mocker):
    ...
    mocker.patch('src.core.repositories.get_professor_by_id', return_value=fake_professor) # <-- MUDANÇA AQUI
    mocker.patch('src.core.repositories.get_project_by_id', return_value=fake_project) # <-- MUDANÇA AQUI
    mocker.patch('src.core.repositories.check_if_orientador_is_assessor', return_value=True) # <-- MUDANÇA AQUI
    mock_create_assoc = mocker.patch('src.core.repositories.create_assessor_assoc') # <-- MUDANÇA AQUI
...
def test_associate_assessor_fails_if_project_not_found(mocker):
    ...
    # 1. ARRANGE
    # --- CAMINHOS DO PATCH CORRIGIDOS ---
    mocker.patch('src.core.repositories.get_professor_by_id', return_value=mocker.MagicMock()) # <-- MUDANÇA AQUI
    mocker.patch(
        'src.core.repositories.get_project_by_id', # <-- MUDANÇA AQUI
...
def test_deactivate_participant_success(mocker):
    ...
    """
    from src.core.services import deactivate_project_participant # <-- IMPORT AQUI
    ...
    mock_queryset.get.return_value = fake_relation
    
    # --- CAMINHOS DO PATCH CORRIGIDOS ---
    mocker.patch('src.core.repositories.get_project_by_id', return_value=mocker.MagicMock()) # <-- MUDANÇA AQUI
    mocker.patch('src.core.repositories.get_active_participant_relation', return_value=mock_queryset) # <-- MUDANÇA AQUI
    mock_save = mocker.patch('src.core.repositories.save_instance') # <-- MUDANÇA AQUI
...
def test_deactivate_participant_fails_invalid_role(mocker):
    ...
    # 2. ACT & 3. ASSERT
    with pytest.raises(ValidationError) as e:
        deactivate_project_participant(project_id=1, role='coordenador')
...
def test_associate_aluno_success(mocker):
    ...
    """
    from src.core.services import associate_aluno_to_project # <-- IMPORT AQUI
    
    # 1. ARRANGE
    fake_project = mocker.MagicMock()
    fake_aluno = mocker.MagicMock()
    mock_assoc = mocker.MagicMock() 

    # --- CAMINHOS DO PATCH CORRIGIDOS ---
    mock_get_proj = mocker.patch('src.core.repositories.get_project_by_id', return_value=fake_project) # <-- MUDANÇA AQUI
    mock_get_aluno = mocker.patch('src.core.repositories.get_aluno_by_id', return_value=fake_aluno) # <-- MUDANÇA AQUI
    mock_create_assoc = mocker.patch('src.core.repositories.create_alunoproj_assoc', return_value=mock_assoc) # <-- MUDANÇA AQUI
...
def test_associate_aluno_fails_on_creation(mocker):
    ...
    # --- CAMINHOS DO PATCH CORRIGIDOS ---
    mocker.patch('src.core.repositories.get_project_by_id', return_value=fake_project) # <-- MUDANÇA AQUI
    mocker.patch('src.core.repositories.get_aluno_by_id', return_value=fake_aluno) # <-- MUDANÇA AQUI
    mocker.patch(
        'src.core.repositories.create_alunoproj_assoc', # <-- MUDANÇA AQUI
...
def test_associate_orientador_success(mocker):
    ...
    """
    from src.core.services import associate_orientador_to_project # <-- IMPORT AQUI
    ...
    mock_get_proj = mocker.patch('src.core.repositories.get_project_by_id', return_value=fake_project) # <-- MUDANÇA AQUI
    mock_get_prof = mocker.patch('src.core.repositories.get_professor_by_id', return_value=fake_professor) # <-- MUDANÇA AQUI
    mock_create_assoc = mocker.patch('src.core.repositories.create_orientador_assoc', return_value=mock_assoc) # <-- MUDANÇA AQUI
...
def test_associate_orientador_fails_on_creation(mocker):
    ...
    mocker.patch('src.core.repositories.get_professor_by_id', return_value=fake_professor) # <-- MUDANÇA AQUI
    mocker.patch(
        'src.core.repositories.create_orientador_assoc', # <-- MUDANÇA AQUI
        side_effect=ValidationError("Erro de validação simulado")
    )
...
def test_link_mongo_to_project_success(mocker):
    ...
    """
    from src.core.services import link_mongo_to_project # <-- IMPORT AQUI
    ...
    # --- CAMINHOS DO PATCH CORRIGIDOS ---
    mock_get_proj = mocker.patch('src.core.repositories.get_project_by_id', return_value=fake_project) # <-- MUDANÇA AQUI
    mock_save = mocker.patch('src.core.repositories.save_instance', return_value=fake_project) # <-- MUDANÇA AQUI
...
def test_save_corretor_text_success(mocker):
    ...
    """
    from src.core.services import save_corretor_text # <-- IMPORT AQUI
